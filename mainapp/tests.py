...
class TestTextViewSet(APITestCase):
    ...
    def test_check_work_count(self):
        ...
        text = Text.objects.create(title='text title', text='test text')
        response = self.client.get(f'{self.url}{text.id}/')
        self.assertEqual(response.data['counter'], first_count_value)

        response = self.client.get(f'{self.url}{text.id}/')
        self.assertEqual(response.data['counter'], second_count_value)
